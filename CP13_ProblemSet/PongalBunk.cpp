...
#define matPrint(a, n, m)                  \
    loop(i, n)                             \
    {                                      \
        loop(j, m) cout << a[i][j] << " "; \
        cout << nline;                     \
    }
#define SquarematInp(a, n, m) \
    i1(n);                    \
    vvi_size(a, n, n);        \
    loop(i, n) { loop(j, m) cin >> a[i][j]; }
#define SquarematPrint(a, n)               \
    loop(i, n)                             \
    {                                      \
        loop(j, n) cout << a[i][j] << " "; \
        cout << nline;                     \
...
void solve()
{
    ...
    i2(n, q)
        ...
        vector<int>
            A(n, 0);
    vector<int> scanline_index(n + 1, 0);
    vector<int> scanline_value(n + 1, 0);
    ...
    while (q--)
    {
        ...
        r--;
        scanline_index[l] += 1;
        scanline_index[r + 1] -= 1;
        scanline_value[r + 1] -= (r - l + 1);
    }

    for (int i = 1; i <= n; i++)
    {
        scanline_index[i] += scanline_index[i - 1];
    }

    for (int i = 1; i <= n; i++)
    {
        scanline_value[i] += (scanline_value[i - 1] + scanline_index[i]);
    }
